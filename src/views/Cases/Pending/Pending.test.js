...
describe('Pending component', () => {
  ...
  test('If Pending Cases renders',()=>{
    const wrapper = shallow(<Pending/>);
    expect(wrapper.exists()).toBe(true);
  })
  test('Pending Cases if renders no li elements would exists', () => {
    const wrapper = mount(
      <I18nextProvider i18n={i18n}>
        <Pending handleCaseStatus={() => {
          }} kc={mockKcProps}/>
      </I18nextProvider>
    );
  ...
  test('Pending Cases have certain states', () => {
    const wrapper = mount(
      <I18nextProvider i18n={i18n}>
        <Pending handleCaseStatus={() => {
          }} kc={mockKcProps}/>
    ...
    const PAGE_LIMIT = 10;
    //console.log(wrapper.debug());
    expect(wrapper.find('Pending').state().loading).toBe(true)
    expect(wrapper.find('Pending').state().totalCases).toBe(0)
    expect(wrapper.find('Pending').state().limit).toBe(PAGE_LIMIT)
    expect(wrapper.find('Pending').state().data).toEqual(null)
    expect(wrapper.find('Pending').state().activePage).toEqual(1)
  })
  ...
  test('if componentDidMount render values correctly', () => {
    ...
    wrapper.update()
    const PAGE_LIMIT = 10;
    expect(mockAxios.get).toHaveBeenCalledWith('cases?status=3&start=1&limit=10', mockHeader);
    expect(wrapper.find('Pending').state().totalCases).toEqual(1);
    expect(wrapper.find('Pending').state().loading).toBe(false);
    expect(wrapper.find('Pending').state().limit).toBe(PAGE_LIMIT);
